...
GaussianBlur::GaussianBlur(int radius, float sigma) :
            radius_ (radius),
            diviation_ (sigma),
...
void GaussianBlurEffect::initBlur(int radius, float sigma, int iteration)
{
    this->radius = radius;
    this->sigma = sigma;
...
QImage GaussianBlurEffect::ApplyBlur(QImage input)
{
    GaussianBlur *gaussianObj = new GaussianBlur(this->radius, this->sigma);

    for(int i = 0 ; i< this->iteration ; i++)
    {
        input = QImage(gaussianObj->ApplyGaussianFilterToImage(input));
    }
